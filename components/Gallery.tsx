    ...
    <section id="gallery" className="py-16 bg-white">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 auto-rows-[150px] md:auto-rows-[180px]">
          {imgs.map((img, i) => (
            <article key={img.id} className={cn('relative rounded-xl overflow-hidden cursor-pointer group bg-cream', grid[i % grid.length])}>
              <img
                src={img.src || '/assets/gallery-1.jpg'}
                alt={img.alt || 'Galeria'}
                className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform"
                onError={(e) => { (e.target as HTMLImageElement).src = '/assets/gallery-1.jpg'; }}
              />
              <button onClick={() => { setIdx(i); setLightbox(true); }} className="absolute inset-0 hover:bg-black/20 transition-colors z-10" />
            </article>
          ))}
        </div>
      </div>
      ...
      {lightbox && imgs[idx] && (
        <div className="fixed inset-0 bg-black/95 flex items-center justify-center z-[500]" onClick={() => setLightbox(false)}>
          ...
          <img src={imgs[idx]?.src || '/assets/gallery-1.jpg'} alt="" className="max-h-[80vh] max-w-[90vw] object-contain" onClick={e => e.stopPropagation()} onError={(e) => { (e.target as HTMLImageElement).src = '/assets/gallery-1.jpg'; }} />
