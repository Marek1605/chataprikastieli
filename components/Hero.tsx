...
export default function Hero() {
  ...
  const h = data.hero;
  const isB64 = h.backgroundImage.startsWith('data:');
  const scroll = (id: string) => (e: React.MouseEvent) => { e.preventDefault(); document.getElementById(id)?.scrollIntoView({ behavior: 'smooth' }); };
  ...
  return (
    <section id="hero" className="relative min-h-[100svh] flex items-center justify-center overflow-hidden">
      <div className="absolute inset-0 z-0">
        ...
        {isB64 ? <img src={h.backgroundImage} alt="" className="absolute inset-0 w-full h-full object-cover" /> : <Image src={h.backgroundImage} alt="" fill priority className="object-cover" sizes="100vw" quality={85} />}
      </div>
      <div className={`relative z-10 text-center px-4 max-w-4xl mx-auto pt-20 pb-24 transition-all duration-1000 ${loaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-8">
          {h.badges.map((b, i) => <span key={i} className="inline-flex items-center gap-1.5 px-4 py-2 bg-white/15 backdrop-blur-sm rounded-full text-sm text-white"><span>{b.icon}</span><span>{b.text}</span></span>)}
        </div>
        ...
        <div className="flex items-center justify-center gap-4 text-white/90">
          <div className="flex items-center gap-2"><span className="text-yellow-400">★★★★★</span><span className="font-semibold">{h.rating}</span></div>
