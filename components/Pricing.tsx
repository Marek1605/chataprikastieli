        ...
        <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {pkgs.map(pkg => (
            <div key={pkg.key} className={`bg-white rounded-2xl p-6 shadow-sm relative ${pkg.popular ? 'ring-2 ring-wood' : ''}`}>
              {pkg.popular && (
                <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-wood text-white text-xs px-3 py-1 rounded-full">
                  {p.popularText}
                </span>
              )}
              ...
              <div className="text-center mb-6">
                ...
                <span className="text-gray-500">/ {pkg.nights} {pkg.nights < 5 ? 'noci' : 'nocí'}</span>
              </div>
              <button 
                onClick={scroll} 
                className={`w-full py-3 rounded-xl font-semibold transition-colors ${pkg.popular ? 'bg-wood text-white hover:bg-wood-dark' : 'bg-gray-100 text-graphite hover:bg-gray-200'}`}
              >
                {p.ctaText}
              </button>
            </div>
